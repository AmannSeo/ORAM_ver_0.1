      ...
      <Grid container spacing={1.5} alignItems="flex-end">
        ...
        <Grid item xs={12} sm={6} lg={1.8}>
          <FormControl size="small" fullWidth>
            ...
            <Select
              value={props.filterSaas}
              label="SaaS"
              onChange={(event) => {
                props.setFilterSaas(event.target.value as SaasType | '');
                props.setPage(0);
              }}
            >
              <MenuItem value="">전체</MenuItem>
        ...
        <Grid item xs={12} sm={6} lg={2.6}>
          <FormControl size="small" fullWidth disabled={!hasDepartmentOptions}>
            ...
            <Select
              value={hasDepartmentOptions ? props.filterDept : ''}
              label="부서"
              onChange={(event) => {
                props.setFilterDept(event.target.value);
                props.setPage(0);
              }}
            >
              <MenuItem value="">{hasDepartmentOptions ? '전체' : '-'}</MenuItem>
